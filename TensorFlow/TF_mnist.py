...
W1_init = np.random.randn(I, L1) / np.sqrt(I)
W2_init = np.random.randn(L1, O) / np.sqrt(L1)

WEIGHT1 = tf.Variable(W1_init.astype(np.float32))
BIAS1 = tf.Variable(tf.zeros([L1]))
WEIGHT2 = tf.Variable(W2_init.astype(np.float32))
BIAS2 = tf.Variable(tf.zeros([O]))
